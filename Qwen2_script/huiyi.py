    ...
    with torch.no_grad():
        try:
            outputs = model.generate(
                ...
                max_new_tokens=50,  # トークン数を減らしてメモリ使用量を削減
            ...
            outputs = model.generate(
                inputs.input_ids, 
                attention_mask=inputs.attention_mask, 
                max_new_tokens=25,  # トークン数をさらに減らして再試行
                num_beams=2, 
                early_stopping=True
            )
            response = tokenizer.decode(outputs[0], skip_special_tokens=True)
...
def simulate_conversation(topic):
    ...
    agent_a_prompt = f"議題: {topic}\nあなたの名前はAIエージェントAです。議題について意見を述べてください。"
    agent_b_prompt = "あなたの名前はAIエージェントBです。エージェントAの意見に対してコメントし、議論を続けてください。"
...
def create_report(conversation_history):
    ...
    summary = generate_response(model, tokenizer, summary_prompt, device)
